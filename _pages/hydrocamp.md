---
layout: course_hydrocamp 
permalink: /hydrocamp/
title: Hydrocamp
nav: true
nav_order: 4

schedule:
  - date: Jul 19
    am: "**Lec 1**: Orientation / Field Site Introduction / Dye Tracing Fundamentals"
    pm: "**Lab 1**: Standard dye solution production"
    data: Standard dye sample scans
    due: "[**Report 1**: Hydrograph analysis](https://www.dropbox.com/scl/fi/igvttjsaopn3xpnxi7dq1/Report-1_hydrograph.docx?rlkey=zetr4zm5phu3moy7hh5czovi1&st=avn9limx&dl=0)"

  - date: Jul 20
    am: "**Field Day 1**: Dye injection / Field trip"

  - date: Jul 21
    am: "**Lec 2**: Dissolved-gas quantification (headspace equilibration) / GC theory"
    pm: "**Lab 2**: Instrument practice (GC) / N<sub>2</sub>O calibration curve estimation"
    data: N<sub>2</sub>O standard scans
    due: "[**Report 2**: Dye calibration curve](https://www.dropbox.com/scl/fi/1pvh79mw8pzee1caeljns/Report-2_calibration_curve_dye.docx?rlkey=ylm5f936rlii7dlnjudr8oxj8&st=gx9a1zo3&dl=0)"

  - date: Jul 22
    am: "**Field Day 2**: YSI calibration / Well water sampling / Dye sample collection"
    data: YSI readings
    due: "[**Report 3**: N<sub>2</sub>O calibration curve](https://www.dropbox.com/scl/fi/bq9xzdx8p5kpr14jvtzah/Report-3_calibration_curve_N2O.docx?rlkey=2a8qpap7fyk6j27gk8t1jrre5&st=hfjaif6s&dl=0)"

  - date: Jul 23
    am: "**Lec 3**: N<sub>2</sub>O cycling in karst / Water isotopes in hydrology"
    pm: "**Lab 3**: N<sub>2</sub>O field sample processing"
    data: N<sub>2</sub>O sample scans
    due: "[**Report 4**: YSI data analysis](https://www.dropbox.com/scl/fi/9arp9jtodqww2rhg86hmd/Report-4_YSI.docx?rlkey=v7edwwhjyjxi8f3qkf4rkcde0&st=f2e8502r&dl=0)"

  - date: Jul 26
    am: "**Lec 4**: Well hydraulics"
    pm: "**Lab 4**: Previous slug test data analysis (Well 3)"
    data: Previous slug test data
    due: "[**Report 5**: N<sub>2</sub>O field data analysis](https://www.dropbox.com/scl/fi/oyjykb05ytiv4dmcxhvx4/Report-5_field_N2O.docx?rlkey=l5lprhrcweme3rt1g76qd50dg&st=eazhxw68&dl=0)"

  - date: Jul 27
    am: "**Field Day 3**: Slug tests (Wells 4–7) / Water tape measurements / RTK survey / Dye sample collection"
    data: Slug test data (Wells 4–7) / Water tape data / Diver data / Elevation (RTK) data

  - date: Jul 28
    am: "**Lec 5**: Stream flow gauging"
    pm: "**Lab 5**: Previous stream flow gauging data analysis"
    data: Water isotope data 
    due: "[**Report 6**: Slug test analysis / Well water table analysis](https://www.dropbox.com/scl/fi/0b3dhn4lgn0dpws2gacvy/Report-6_slug_test.docx?rlkey=nkhegn0kihxy0yiv6ky20cjom&st=atqwlyve&dl=0)"

  - date: Jul 29
    am: "**Field Day 4**: Stream gauging / Dye sample collection"
    data: Stream gauging data

  - date: Jul 30
    am: "**Lec 6**: Fluorescence spectroscopy"
    pm: "**Lab 6**: Dye field sample scanning (Shimadzu RF-6000)"
    data: Dye spectral data (Field Days 1–4)
    due: "[**Report 7**: Stream flow analysis](https://www.dropbox.com/scl/fi/r2o71xc2w1to2vxuvhv1j/Report-7_stream_flow_gauging.docx?rlkey=bn92pvnzugenjbe812i21z9d3&st=8bi3lxe2&dl=0)"

  - date: Aug 2
    am: "**Lec 7**: Charcoal Packet Analysis"
    pm: "**Lab 7**: Charcoal Packet Elution and Scanning"
    data: Charcoal eluate spectral data

  - date: Aug 3
    am: "**Field Day 5**: Dye sample collection / Field gear wrap-up"
    data: Dye sample, charcoal pacekt, filed fluorometer data

  - date: Aug 4
    am: "Work on Report 8"
    due: "[**Report 8**: Breakthrough curve analysis](https://www.dropbox.com/scl/fi/qxadoeqvsi1brtq3ja414/Report-8_breakthrough_cuve.docx?rlkey=rju9evpiia77s3u07cvon2iy2&st=vei11m22&dl=0)"

  - date: Aug 5
    am: "Work on Final Report"
    due: "[**Final Report**](https://www.dropbox.com/scl/fi/4wkphtv76lv7kgkv2vpxl/Final-report.docx?rlkey=nxurqz6nisq2zh9vu727sv48h&st=omnem8ya&dl=0)"

toc:
  sidebar: left

images:
  slider: true

cohort_images:
  - assets/img/hydrocamp26_students.jpg
  - assets/img/hydrocamp26_instructors.jpg
---

## Overview 
This 2-credit intensive field- and lab-based course introduces students to groundwater flow, karst hydrogeology, and surface water-groundwater interactions through classroom instruction, laboratory exercises, and multi-day field investigations. Students will collect, analyze, and synthesize hydrogeologic and hydrochemical data into professional technical reports. These concepts and practical skills are directly relevant to careers in environmental and consulting work and help prepare students for the Professional Geologist (PG) licensure exam. 
- **Instructor**: Dr. Sean Yoon
- **Filed Site**: O’Leno State Park and River Rise State Preserve


## Application (Summer B 2027)
Please fill out the [Google Form](https://docs.google.com/forms/d/e/1FAIpQLSeQhANx33MncCMDL9PwSQ5YlUATS8KAyLtmUD9rw5gA8GR66A/viewform?usp=publish-editor) to apply!

## Goals
- Quantitatively interpret surface-water and groundwater dynamics using stream-gauging measurements and pressure-transducer data.
- Design, conduct, and interpret dye-tracing and hydrogeochemical field investigations.
- Understand well hydraulics and design, conduct, and interpret aquifer and well tests.
- Analyze and interpret fluorescence, gas chromatography (GC), stable isotope, and in situ sensor data.
- Integrate multiple data sources into clear, professional technical reports.

## Format
This course combines classroom lectures, laboratory practice, fieldwork, and technical reporting. The course will meet either on campus (mostly in classrooms and laboratories) or at field sites.  
- **Classroom Lectures** introduce theoretical concepts and field methods.  
- **Laboratory practices** include sampling preparation, calibration, surveying, and data analysis.  
- **Fieldwork** includes four full field days of groundwater, hydrochemical, and tracer investigations. Students will meet in Williamson Hall in the morning and depart for the field sites (O’Leno State Park and River Rise State Preserve) using a Geological Sciences Department van.
- **Reports** integrate daily data and culminate in a final synthesis report. 

## Prerequisites
- None, but background coursework in hydrogeology and hydrochemistry is recommended.

## Required Materials
- Field notebook (waterproof recommended) 
- Laptop with Microsoft Excel (available free to UF students through Microsoft 365) installed
- Appropriate field clothing and footwear 
- Refillable water bottle and sun/rain protection 

## Textbooks (optional)
- Goldscheider, N., & Drew, D. (Eds.). (2007). Methods in karst hydrogeology: IAH: International Contributions to Hydrogeology, 26.  
A useful reference for students interested in deeper background on field methods and concepts in karst hydrogeology.


## Grading
- Assignments 1 - 8: 5 points each (40 points total) 
- Final Synthesis Report: 40 points 
- Participation in fieldwork and lab activities: 20 points
  - Attendance and punctuality (8 points)
    - Attends all field and lab sessions and arrives prepared and on time.
    - Minor absence/late arrival results in proportional deduction.
  - Active engagement in field/lab tasks (6 points)
    - Contributes to measurements, sampling, equipment setup, and data recording.
    - Demonstrates initiative and willingness to assist team members.
  - Preparation and adherence to safety protocols (3 points)
    - Comes prepared with required materials and follows safety and field protocols.
  - Teamwork and professionalism (3 points)
    - Works collaboratively, communicates respectfully, and contributes to group progress.















